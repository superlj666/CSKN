...
def load_data(dataset_name, batch_size = 128, data_dir = './data', kwargs={'num_workers': 2, 'pin_memory': True}):
    if dataset_name == 'MNIST':
        ...
        train_loader = torch.utils.data.DataLoader(datasets.FashionMNIST(data_dir, train=True, download=True, transform=transform_fashionmnist), batch_size=batch_size, shuffle=True, **kwargs)
        test_loader = torch.utils.data.DataLoader(datasets.FashionMNIST(data_dir, train=False, transform=transform_fashionmnist), batch_size=1000, shuffle=False, **kwargs)
    elif dataset_name == 'SVHN':
        train_loader = torch.utils.data.DataLoader(datasets.SVHN(data_dir, split='train', download=True, transform=transforms.ToTensor()), batch_size=batch_size, shuffle=True, **kwargs)
        test_loader = torch.utils.data.DataLoader(datasets.SVHN(data_dir, split='test', download=True, transform=transforms.ToTensor()), batch_size=1000, shuffle=True, **kwargs)
